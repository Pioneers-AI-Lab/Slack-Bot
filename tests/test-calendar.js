...
async function testCalendar() {
	...
	try {
		...
		const weeklyResult = await fetchCalendarEvents(
			weekly.start,
			weekly.end,
		);
		console.log('   ✅ Success!');
		...
		if (
			dailyResult.events.length > 0 ||
			dailyResult.milestones.length > 0
		) {
			console.log('3. Sample Events (Daily):');
			const allEvents = [
				...dailyResult.milestones,
				...dailyResult.events,
			].slice(0, 5);
		...
		if (dailyResult.milestones.length > 0) {
			...
			console.log('4. Milestone Detection:');
			console.log(
				'   ℹ️  No milestones found in daily range (this is OK)',
			);
		}
		...
		if (error.message.includes('credentials')) {
			...
			console.error(
				'   3. Service account has read access to the calendar',
			);
		}
