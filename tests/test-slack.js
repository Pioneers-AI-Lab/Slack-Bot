...
async function testSlack() {
	...
	console.log(
		'   Admin Channel:',
		process.env.SLACK_ADMIN_CHANNEL_ID || 'Not set (optional)',
	);
	console.log();
	...
	try {
		...
		if (process.env.SLACK_ADMIN_CHANNEL_ID) {
			...
			await notifyAdmin(
				'This is a test admin notification from the integration test suite.',
			);
			console.log('   ✅ Admin notification sent!');
			console.log(
				'   Admin Channel:',
				process.env.SLACK_ADMIN_CHANNEL_ID,
			);
		} else {
			console.log('2. Admin Notification:');
			console.log('   ⚠️  SLACK_ADMIN_CHANNEL_ID not set, skipping');
			console.log(
				"   (This is optional and won't affect main functionality)",
			);
		}
		...
		console.log(
			'   Check your Slack channel to verify the message was posted.',
		);
	} catch (error) {
		...
		if (error.message.includes('token')) {
			...
			console.error(
				'   3. Channel ID starts with C (for public channels)',
			);
		}
